...
def get_activities(name):
	...
	if frappe.db.exists("CRM Pipeline", name):
		return get_pipeline_activities(name)
...
def get_pipeline_activities(name):
    get_docinfo("", "CRM Pipeline", name)
    docinfo = frappe.response["docinfo"]
    pipeline_meta = frappe.get_meta("CRM Pipeline")
    pipeline_fields = {
        field.fieldname: {"label": field.label, "options": field.options} for field in pipeline_meta.fields
    }
    avoid_fields = [
        "response_by",
        "sla_creation",
    ...
    doc = frappe.db.get_values("CRM Pipeline", name, ["creation", "owner", "lead"])[0]
    lead = doc[2]
    ...
    tasks = []
    attachments = []
    creation_text = "created this pipeline"

    if lead:
        activities, calls, notes, tasks, attachments = get_lead_activities(lead)
        creation_text = "converted the lead to this pipeline"
    ...
    for version in docinfo.versions:
        ...
        if change := data.get("changed")[0]:
            field = pipeline_fields.get(change[0], None)
    ...
    notes = notes + get_linked_notes(name) + get_linked_calls(name).get("notes", [])
    tasks = tasks + get_linked_tasks(name) + get_linked_calls(name).get("tasks", [])
    attachments = attachments + get_attachments("CRM Pipeline", name)
...
def get_linked_calls(name):
	...
	if notes:
		notes = frappe.db.get_all(
			...
			fields=["name", "title", "content", "owner", "modified", "custom_note_type"],
		)
