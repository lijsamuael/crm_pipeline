...
const { reload: reloadView, getDefaultView, getView, views } = viewsStore()
...
onMounted(async () => {
  // Wait for views to be ready before loading data
  await nextTick()
  // Ensure data loads even if auto didn't trigger
  if (!list.value?.data && !list.value?.loading) {
    useDebounceFn(reload, 100)()
  }
})

// Watch for views to be ready and reload if needed (for first-time users)
watch(
  () => views.data?.length,
  (viewsLength) => {
    // If views are loaded but list data is not, reload after a short delay
    if (viewsLength > 0 && !list.value?.data && !list.value?.loading) {
      nextTick(() => {
        setTimeout(() => {
          if (!list.value?.data && !list.value?.loading) {
            reload()
