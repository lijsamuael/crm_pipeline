  ...
  >
    ...
    <div class="mt-1 flex items-center justify-between gap-2">
      <div class="flex items-center gap-2 truncate">
        ...
        <template v-if="note.custom_note_type || note.note_type">
          <div class="h-1 w-1 rounded-full bg-ink-gray-5 flex-shrink-0"></div>
          <div class="flex items-center gap-1 px-1.5 py-0.5 rounded bg-surface-gray-2 whitespace-nowrap">
            <div class="truncate text-xs text-ink-gray-8 font-medium">
              {{ note.custom_note_type || note.note_type }}
            </div>
          </div>
        </template>
      </div>
