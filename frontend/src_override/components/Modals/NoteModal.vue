  ...
  <Dialog v-model="show" :options="{ size: 'xl' }">
    ...
    <template #body-content>
      <div class="flex flex-col gap-4">
        ...
        <div>
          <div class="mb-1.5 text-xs text-ink-gray-5">{{ __('Note Type') }}</div>
          <Link
            class="form-control"
            :value="_note.custom_note_type"
            doctype="Note Type"
            @change="(option) => (_note.custom_note_type = option)"
            :placeholder="__('Select Note Type')"
          />
        </div>
        <div>
...
watch(
  ...
  (value) => {
    ...
    nextTick(() => {
      ...
      _note.value = { 
        ...props.note,
        custom_note_type: props.note?.custom_note_type || '',
      }
